...
;(function ( $, window, document, undefined ) {
	...
	methods = {
      ...
      var page = 'page ' + (methods.iSlideCounter + 1) +'/' + config.iNumSlides;
      $('.page').html(page);
      $('#slider-mini').attr('max', config.iNumSlides);
		},
    ...
    moveLeft: function(){
      ...
      var page = 'page ' + (methods.iSlideCounter + 1) + '/' + config.iNumSlides;
      $('.page').html(page);
      var percentage = (methods.iSlideCounter * 100) / (config.iNumSlides-1);
      $('.ui-slider-bg').css('width', percentage+'%');
		},
      ...
      $('#slider-mini').attr('value', methods.iSlideCounter+1);
      $('#slider-mini').change();
		},
    ...
    setSlidePosition: function(slide) {
      var current = methods.iSlideCounter;
      if (slide - current == 0) return;
      else {
        if (slide > current) {
          for (var i=current; i != slide; i++) {
            methods.moveLeft();
          }
        } else {
          for (var i=current; i != slide; i--) {
